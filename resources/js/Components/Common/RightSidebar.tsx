...
const RightSidebar = (props: any) => {
    ...
    // open offcanvas - lần đầu mở, sau đó lưu vào localStorage
    const [open, setOpen] = useState<boolean>(() => {
        try {
            if (typeof window !== 'undefined' && 'localStorage' in window) {
                const saved = window.localStorage.getItem('theme-customizer-open');
                // Nếu đã có trong localStorage thì dùng giá trị đó, nếu chưa thì mở lần đầu
                if (saved !== null) {
                    return saved === 'true';
                } else {
                    // Lần đầu tiên: mở và lưu vào localStorage
                    window.localStorage.setItem('theme-customizer-open', 'true');
                    return true;
                }
            }
        } catch {
        ...
        return true; // Mặc định mở nếu không có localStorage
    });

    const toggleLeftCanvas = () => {
        const newValue = !open;
        setOpen(newValue);
        // Lưu trạng thái đóng/mở vào localStorage
        try {
            if (typeof window !== 'undefined' && 'localStorage' in window) {
                window.localStorage.setItem('theme-customizer-open', String(newValue));
            }
        } catch {
            // Bỏ qua lỗi localStorage
        }
